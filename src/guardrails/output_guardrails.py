...
def content_filter(response: str) -> dict:
    ...
    PII_PATTERNS = {
        ...
        "password_field": r"(?:password|mật\s*khẩu)\s*[:=]\s*\S+",
        "phone": r"(?:\+84|84|0)(?:3|5|7|8|9)\d{8}\b|\b0\d{9,10}\b",
...
async def llm_safety_check(response_text: str) -> dict:
    ...
    try:
        ...
        return {"safe": True, "verdict": f"Judge check skipped due to error: {e}"}
        ...
        *,
        ...
        if self.use_llm_judge and judge_runner is not None:
            ...
            if not judge_res["safe"]:
                ...
                safe_msg = (
                    "I cannot provide that response as it violates security policies."
                )
                llm_response.content = types.Content(
                    role="model",
